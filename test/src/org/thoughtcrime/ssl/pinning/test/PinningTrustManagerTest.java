...
public class PinningTrustManagerTest extends AndroidTestCase {
  ...
  protected void setUp() throws Exception {
    ...
    trustManager =
 new PinningTrustManager(SystemKeyStore.getInstance(),
                                new String[] {"40c5401d6f8cbaf08b00edefb1ee87d005b3b9cd"},
                                0);
  }

    public void testValidChainAndPin() throws CertificateException,
            NoSuchAlgorithmException, KeyStoreException {
        CertificateFactory certificateFactory = CertificateFactory
                .getInstance("X509");
        X509Certificate googleWildcard = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(GOOGLE_WILDCARD
                        .getBytes()));
        X509Certificate googleAuthority = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(GOOGLE_AUTHORITY
                        .getBytes()));
        X509Certificate equifaxRoot = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(EQUIFAX_ROOT
                        .getBytes()));

        X509Certificate[] chain = makeChain(googleWildcard, googleAuthority,
                equifaxRoot);
        trustManager.clearCache();
        trustManager.checkServerTrusted(chain, googleWildcard.getPublicKey()
                .getAlgorithm());
    ...
    public void testValidChainImpliedRootAndPin() throws CertificateException,
            NoSuchAlgorithmException, KeyStoreException {
        CertificateFactory certificateFactory = CertificateFactory
                .getInstance("X509");
        X509Certificate googleWildcard = (X509Certificate) certificateFactory
    ...
    public void testInvalidChainWithValidPin() throws CertificateException,
            NoSuchAlgorithmException, KeyStoreException {
        CertificateFactory certificateFactory = CertificateFactory
                .getInstance("X509");
        X509Certificate verisignEv = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(
                        VERISIGN_CLASS_3_EV.getBytes()));
        X509Certificate googleAuthority = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(GOOGLE_AUTHORITY
                        .getBytes()));
        ...
        try {
            trustManager.checkServerTrusted(chain, verisignEv.getPublicKey()
                    .getAlgorithm());
    ...
    public void testValidChainWithNoPin() throws CertificateException,
            NoSuchAlgorithmException, KeyStoreException {
        CertificateFactory certificateFactory = CertificateFactory
                .getInstance("X509");
        X509Certificate verisignEv = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(
                        VERISIGN_CLASS_3_EV.getBytes()));
        X509Certificate verisignClassThree = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(
                        VERISIGN_CLASS_THREE.getBytes()));
        X509Certificate verisignRoot = (X509Certificate) certificateFactory
                .generateCertificate(new ByteArrayInputStream(VERISIGN_ROOT
                        .getBytes()));

        X509Certificate[] chain = makeChain(verisignEv, verisignClassThree,
                verisignRoot);
        trustManager.clearCache();
    ...
    public void testValidChainWithGhostPin() throws CertificateException,
            NoSuchAlgorithmException, KeyStoreException {
        CertificateFactory certificateFactory = CertificateFactory
                .getInstance("X509");
        X509Certificate verisignEv = (X509Certificate) certificateFactory
